...
class TestNoFaults:
    ...
    def test_a(self, interpolator):
        ...
        geo_data = gempy_lite.create_data(extent=[0, 10, 0, 10, -10, 0], resolution=[50, 50, 50],
        ...
        # Compute model
        sol = gempy_lite.compute_model(geo_data)
        ...
        gempy_lite.plot.plot_2d(geo_data, cell_number=25, direction='y', show_data=True)
        plt.savefig(os.path.dirname(__file__) + '/../figs/test_a.png', dpi=100)

        gempy_lite.plot.plot_2d(geo_data, cell_number=25, show_scalar=True)
    ...
    def test_b(self, interpolator):
        ...
        # Checking that the plots do not rise errors
        gempy_lite.plot.plot_2d(geo_data, cell_number=25)
...
class TestFaults:
    ...
    def test_d(self, interpolator):
        ...
        gempy_lite.map_stack_to_surfaces(geo_data, {'fault1': 'f1', 'series': ('A', 'B')})
    ...
    def test_f_sort_surfaces(self, interpolator):
        ...
        # Importing the data from csv files and settign extent and resolution
        geo_data = gempy_lite.create_data(extent=[0, 2000, 0, 2000, -2000, 0], resolution=[50, 50, 50],
                                          path_o=input_path + "/GeoModeller/test_f/test_f_Foliations.csv",
                                          path_i=input_path + "/GeoModeller/test_f/test_f_Points.csv")

        gempy_lite.map_stack_to_surfaces(geo_data, {'fault1': 'MainFault',
                                                'series': ('Reservoir',
                                                           'Seal',
                                                           'SecondaryReservoir',
                                                           'NonReservoirDeep'
        ...
        # Compute model
        sol = gempy_lite.compute_model(geo_data, sort_surfaces=True)
        ...
        gempy_lite.compute_model(geo_data)
        gempy_lite.plot.plot_2d(geo_data, cell_number=25, direction='y', show_data=True)
        plt.show()
        ...
        ver, sim = gempy_lite.get_surfaces(geo_data)
        print(ver, sim)
